...
model_path = "D:\\cccc\\basu_dj\\landmark_gnn_model1.pth"
model = LandmarkGNN(in_channels=3, hidden_channels=64, out_channels=3)
...
data_path = "D:\\cccc\\basu_dj\\landmark_test\\faces\\"
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            for filename in os.listdir(data_path):
                if filename.endswith(".npy"):
                    ...
                    #print(saved_data)
                    #print(output)
                    if similarity.item() >= 0.995:
                        print(f"High similarity ({similarity.item():.2f}%) with file: {filename}")
                    distance = torch.sum(torch.abs(output - saved_data))
                    print(distance,filename)
                    if distance < 2e+22:
                         print(f"High similarity with file: {filename}")
